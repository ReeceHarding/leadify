...
export default function LeadGenerationDebugger() {
  ...
  const updateStep = (name: string, status: DebugStep["status"], message?: string, data?: any) => {
  ...
  const runDiagnostics = async () => {
    ...
    try {
      ...
      if (user && user.id) {
        updateStep("User Authentication", "success", "User authenticated", { userId: user.id })
      } else {
      ...
      if (keywords.length > 0) {
        updateStep("Keywords Check", "success", `Found ${keywords.length} keywords`, { keywords })
      } else {
      ...
      if (campaignsResult.isSuccess && campaignsResult.data.length > 0) {
        ...
        updateStep("Campaign Check", "success", `Found ${campaigns.length} campaigns`, {
        ...
        for (const campaign of campaigns) {
          const commentsResult = await getGeneratedCommentsByCampaignAction(campaign.id)
          if (commentsResult.isSuccess) {
            totalComments += commentsResult.data.length
            addLog(`Campaign ${campaign.id} has ${commentsResult.data.length} comments`)
          }
        }
        updateStep("Comments Check", totalComments > 0 ? "success" : "error", 
          totalComments > 0 ? `Found ${totalComments} total comments` : "No comments found in any campaign"
        )
      } else {
        updateStep("Campaign Check", "error", "No campaigns found")
        updateStep("Comments Check", "error", "Cannot check comments without campaigns")
      }
      ...
      addLog("Diagnostic error", { error: error instanceof Error ? error.message : "Unknown error" })
  ...
  const createTestComment = async () => {
    ...
    try {
      ...
      const testComment = {
        ...
        postContentSnippet: "I'm looking for recommendations on the best tools for my business. We need something that can help with...",
        relevanceScore: 85,
        reasoning: "This is a test lead with high relevance score for testing purposes",
        microComment: "Check out our solution at example.com!",
        mediumComment: "Based on your needs, I'd recommend checking out our platform. It's specifically designed for businesses like yours and has helped many similar companies streamline their operations.",
          ...
          {steps.map((step, index) => (
            <div key={index} className="flex items-center justify-between rounded-lg border p-3">
              <div className="flex items-center gap-3">
                {step.status === "pending" && <Bug className="text-muted-foreground size-4" />}
                {step.status === "checking" && <Loader2 className="size-4 animate-spin text-blue-500" />}
                ...
                <div>
                  ...
                  {step.message && (
                    <div className="text-muted-foreground text-sm">{step.message}</div>
                  )}
                </div>
              </div>
              <Badge variant={
      ...
      <Card>
        ...
        <CardContent>
          <ScrollArea className="h-96 w-full rounded-md border p-4 font-mono text-xs">
            {logs.length === 0 ? (
              <div className="text-muted-foreground">No logs yet. Run diagnostics to see logs.</div>
            ) : (
              logs.map((log, index) => (
          ...
          <Alert>
            ...
            <AlertDescription>
              <ul className="mt-2 list-inside list-disc space-y-1">
                <li><strong>No comments showing:</strong> Click "Create Test Comment" to verify the UI works</li>
                <li><strong>Campaign not found:</strong> Click "Create Test Campaign" to create one</li>
                <li><strong>Workflow not running:</strong> Click "Run Full Workflow" to start lead generation</li>
                <li><strong>No keywords:</strong> Complete onboarding at /onboarding</li>
              </ul>
            </AlertDescription>
          </Alert>
        </CardContent>
