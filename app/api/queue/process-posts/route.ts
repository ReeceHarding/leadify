...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    for (const docSnapshot of snapshot.docs) {
      ...
      console.log(`📝 [QUEUE-PROCESSOR] Processing post ${postId} for lead ${postData.leadId}`)
      
      try {
        // Update status to processing
        await updateDoc(doc(db, POSTING_QUEUE_COLLECTIONS.POSTING_QUEUE, postId), {
          status: 'processing',
        ...
        if (result.isSuccess) {
          console.log(`✅ [QUEUE-PROCESSOR] Successfully posted comment for lead ${postData.leadId}`)
          ...
          console.error(`❌ [QUEUE-PROCESSOR] Failed to post: ${result.message}`)
          ...
          await updateDoc(doc(db, POSTING_QUEUE_COLLECTIONS.POSTING_QUEUE, postId), {
            ...
            scheduledFor: newRetryCount < maxRetries 
        ...
        console.error(`❌ [QUEUE-PROCESSOR] Error processing post ${postId}:`, error)
    ...
    console.log(`🏁 [QUEUE-PROCESSOR] Completed processing. Processed: ${processedCount}, Errors: ${errors.length}`)
