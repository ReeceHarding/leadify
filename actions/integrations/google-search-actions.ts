...
export async function searchRedditThreadsAction(
  ...
  try {
    ...
    const results: RedditSearchResult[] = data.items.map((item: any, index: number) => {
      ...
      const redditUrlMatch = result.link.match(/\/r\/([^\/]+)\/comments\/([^\/]+)/)
      if (redditUrlMatch) {
    ...
    console.log(`✅ Found ${redditResults.length} Reddit threads for keyword: "${keyword}"`)
...
export async function searchMultipleKeywordsAction(
  ...
  try {
    ...
    for (const keyword of keywords) {
      const searchResult = await searchRedditThreadsAction(keyword, maxResultsPerKeyword)
      ...
      if (searchResult.isSuccess) {
        ...
        console.error(`Failed to search for keyword "${keyword}":`, searchResult.message)
...
export async function testGoogleSearchConnectionAction(): Promise<ActionState<{ status: string }>> {
  try {
    if (!process.env.GOOGLE_SEARCH_API_KEY || !process.env.GOOGLE_SEARCH_ENGINE_ID) {
      return { 
        isSuccess: false, 
