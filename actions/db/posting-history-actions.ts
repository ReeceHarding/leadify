...
interface PostingHistoryDocument {
  id: string
  userId: string
  organizationId?: string // NEW: Organization-based posting history
  subreddit: string
...
export async function getSubredditPostingHistoryByOrganizationAction(
  organizationId: string,
  subreddits: string[]
): Promise<ActionState<SubredditPostingHistory[]>> {
  try {
    console.log(
      "📊 [POSTING-HISTORY] Getting history for organization:",
      organizationId,
      "subreddits:",
      subreddits
    )
...
export async function getSubredditPostingHistoryAction(
  ...
  try {
    console.log(
      "📊 [POSTING-HISTORY] Getting history for subreddits (LEGACY):",
      subreddits
    )
...
export async function updatePostingHistoryByOrganizationAction(
  organizationId: string,
  subreddit: string
): Promise<ActionState<void>> {
  try {
    console.log("📊 [POSTING-HISTORY] Updating history for organization:", {
      organizationId,
      subreddit
    })

    // Create document ID from organizationId and subreddit
    const docId = `${organizationId}_${subreddit}`
...
export async function updatePostingHistoryAction(
  ...
  try {
    console.log("📊 [POSTING-HISTORY] Updating history for (LEGACY):", {
      userId,
      subreddit
    })
