...
export async function generateKeywordsAction({
  ...
  try {
    ...
    const prompt = `system:\nYou are a growth hacker. Goal: surface Reddit conversations where users expose intent for the business below.\nThink in 3 steps and OUTPUT JSON only.\n1) 🤔 Reason about language patterns target users use.\n2) 🧪 Generate candidate queries (each with reach & signal estimate 0-1). Produce N×2 queries (broad & narrow) where N=${requestedCount}.\n3) 🩺 Score candidate with harmonic_mean(signal, log10(reach+1)).\nReturn {\n  "candidates":[{\n      "q":"string",\n      "reach":0.0,\n      "signal":0.0,\n      "score":0.0,\n      "rationale":"string"\n  }...]\n}\nuser:\n${businessInfo}\n${refinement}`
